...
import ctypes
import os
import keyboard  # Ensure you have this import
...
def block_input():
    ...
    print(f"Keys {blocked_keys} blocked")

def unblock_input():
    """Unblock specific keys."""
    for key in blocked_keys:
        keyboard.unblock_key(key)
    print(f"Keys {blocked_keys} unblocked")
...
def on_login_click(event=None):
    ...
    if USERNAME:
        ...
        show_taskbar()
        unblock_input()  # Unblock keys after successful login
...
def show_login_screen():
    ...
    hide_taskbar()
    block_input()  # Block keys on login screen
...
def update_staff_buttons():
    """Update the state of StaffIn and StaffOut buttons based on STAFF_IN_TIME."""
    if STAFF_IN_TIME:
...
def show_task_management_screen(username, user_id):
    ...
    update_staff_buttons()  # Call to set the initial button states
...
def save_staff_in_time():
    ...
    try:
        ...
        pass
    update_staff_buttons()  # Update button states after saving staff in time
...
def update_staff_out_time():
    ...
    try:
        ...
        pass
    update_staff_buttons()  # Update button states after updating staff out time
...
def ensure_end_task_buttons():
    for task_id, task in RUNNING_TASKS.items():
        print(f"Ensuring button for task {task_id}")
        ...
        running_task_treeview_reference.add_button(str(task_id))  # Ensure button is added
...
def fetch_task_timers():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            RUNNING_TASKS.clear()  # Clear existing tasks to avoid duplication
            for task in task_timers:
            ...
            ensure_end_task_buttons()  # Add end task buttons after fetching tasks
            return task_timers
    except requests.exceptions.RequestException as e:
...
def update_task_list():
    ...
    ensure_end_task_buttons()  # Ensure buttons are placed
...
class CustomTreeview(ttk.Treeview):
    ...
    def add_button(self, item):
        if item not in self.buttons:
            print(f"Adding button for item {item}")
    ...
    def place_button(self, item, retry_count=0):
        if item in self.buttons:
            ...
            if bbox:
                print(f"Placing button for item {item} at {bbox}")
                btn.place(x=bbox[0] + 2, y=bbox[1] + 2)
            else:
                print(f"Failed to place button for item {item}, bbox is None. Retrying...")
